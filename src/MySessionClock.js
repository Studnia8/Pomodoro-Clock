    ...
    lengthControl(stateToChange, sign, currentLength, clockType) {
        ...
        if (this.state.clockType === clockType) {
          if (sign === "-" && currentLength !== 0 ) {
            this.setState({[stateToChange]: currentLength - 15});
        ...
        return(
            <div className="app">
                ...
                <div id="mainClock">
                    ...
                    <div className="countdown" style={this.state.timeColor}>
                    {this.clockify(this.state.time)}
                    </div>
                </div>
                <div id="mainClockControl">
                    <FaPlay className="buttonControl" id="play"
                    onClick={this.launch}/>
                    <FaPause className="buttonControl" id="pause"
                    onClick={this.clockStateControl}/>
                    <FaRedoAlt
                    className="buttonControl" id="reset"
                    onClick={this.reset}/>
                </div>
                <div className="madeby">Made by <a className="madeLink" title="Go to Github!" target="_blank" rel="noopener noreferrer" href="https://github.com/Studnia8">Studnia8</a>
                </div>
                <audio id="alarm" preload="auto" src="https://goo.gl/65cBl1" ref={(audio) => { this.alarmSound = audio; }} />
            </div>
        )
    }
}
